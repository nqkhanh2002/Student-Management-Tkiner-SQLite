...
def professor_register():
    ...
    screen_width = Pf_RegisterForm.winfo_screenwidth()
    screen_height = Pf_RegisterForm.winfo_screenheight()

    x_cordinate = int((screen_width/2) - (window_width/2))
    y_cordinate = int((screen_height/3) - (window_height/3))
    Pf_RegisterForm.geometry("{}x{}+{}+{}".format(window_width, window_height, x_cordinate, y_cordinate))
    Pf_RegisterForm.resizable(0,0)
    Pf_RegisterForm.title("TRANG CHỦ QUẢN TRỊ")
    Pf_RegisterForm.config(bg='white')
    AHframe = Frame(Pf_RegisterForm, height=40, width=1280, bg='#3C6739').place(relx=0, y=0)
    AHHeading = Label(Pf_RegisterForm, text='ĐĂNG KÝ GIẢNG VIÊN', font=('Roboto', 18), bg='#3C6739', fg='white').place(
            relx=0.4, rely=0)
    Pf_RegisterForm.Left = ttk.Frame(Pf_RegisterForm, width=300, height=1000, relief="raise")
    Pf_RegisterForm.Left.place(x=2, y=50)
    lbl = Label(Pf_RegisterForm, text="ĐĂNG KÝ GIẢNG VIÊN MỚI", font=("Roboto", 10, 'bold'), bg='white')
    lbl.place(x=25, y=68)
    global ent_pf_no
    global ent_pf_name
    global ent_pf_code
    ...
    lbl_reg_no = Label(Pf_RegisterForm, text="ID", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                              y=100)
    lbl_pf_name = Label(Pf_RegisterForm, text="Tên giảng viên", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=135)
    lbl_pf_code = Label(Pf_RegisterForm, text="Tên đăng nhập", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=170)
    lbl_Pf_pword = Label(Pf_RegisterForm, text="Mật khẩu", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=205)
    lbl_pf_course_flag = Label(Pf_RegisterForm, text="Mã khóa học", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=240)
    lbl_pf_st_flag = Label(Pf_RegisterForm, text="Mã sinh viên", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=275)
    ...
    btn_show = ttk.Button(Pf_RegisterForm, text="Làm mới", command=Read_pf, width = 20).place(x = 40,y = 320)
    #btn_show.pack()
    btn_add = ttk.Button(Pf_RegisterForm, text="Thêm giảng viên mới" ,command=add_book_pf,width = 20).place(x = 40,y = 350)
    #btn_add.pack()
    btn_exit = ttk.Button(Pf_RegisterForm, text="Thoát", command = exit_register_pf,width = 20).place(x = 40,y = 380)
    #btn_exit.pack()
    #goto_TeacherHome
    ...
    tree.heading('pf_name', text="Tên giảng viên", anchor=W)
    tree.heading('pf_code', text="Tên tài khoản", anchor=W)
    tree.heading('pf_pword', text="Mật khẩu", anchor=W)
    tree.heading('pf_subject_flag', text="Mã khóa học", anchor=W)
    tree.heading('pf_student_flag', text="Mã sinh viên", anchor=W)
...
def add_book_pf():
    if ent_pf_no.get() == "" or ent_pf_name.get() == "" or ent_pf_code.get() == "" or ent_pf_pword.get() == "" or ent_pf_sub_flag.get() == "" or ent_pf_st_flag.get() == "":
        messagebox.showinfo("Requirment!", "Vui lòng nhập đầy đủ thông tin!")
    else:
        Database_pf()
...
def student_register():
    ...
    screen_width = st_RegisterForm.winfo_screenwidth()
    screen_height = st_RegisterForm.winfo_screenheight()
    ...
    lbl_st_name = Label(st_RegisterForm, text="Tên sinh viên", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=135)
    lbl_st_code = Label(st_RegisterForm, text="Tên tài khoản", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=170)
    lbl_st_pword = Label(st_RegisterForm, text="Mật khẩu", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=205)
    lbl_st_subject_flag = Label(st_RegisterForm, text="Mã khóa học", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=240)
    lbl_st_professor_flag = Label(st_RegisterForm, text="Mã giảng viên", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=275)
    ...
    btn_show = ttk.Button(st_RegisterForm, text="Làm mới", command=Read_st, width = 20).place(x = 40,y = 320)
    #btn_show.pack()
    btn_add = ttk.Button(st_RegisterForm, text="Thêm mới sinh viên" ,command=add_book_st,width = 20).place(x = 40,y = 350)
    #btn_add.pack()
    btn_exit = ttk.Button(st_RegisterForm, text="Thoát", command = exit_register_st,width = 20).place(x = 40,y = 380)
    #btn_exit.pack()
    #goto_TeacherHome
    ...
    tree.heading('st_code', text="Tên tài khoản", anchor=W)
    tree.heading('st_pword', text="Mật khẩu", anchor=W)
    tree.heading('st_subject_flag', text="Mã môn học", anchor=W)
    tree.heading('st_professor_flag', text="Mã giảng viên", anchor=W)
...
def course_register():
    ...
    screen_width = cr_RegisterForm.winfo_screenwidth()
    screen_height = cr_RegisterForm.winfo_screenheight()
    ...
    lbl_reg_no = Label(cr_RegisterForm, text="ID", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                              y=100)
    lbl_cr_1 = Label(cr_RegisterForm, text="Môn học 1", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=135)
    lbl_cr_2 = Label(cr_RegisterForm, text="Môn học 2", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=170)
    lbl_cr_3 = Label(cr_RegisterForm, text="Môn học 3", font=("Roboto", 8, 'bold'), bg='white').place(x=16,
                                                                                                               y=205)
    lbl_cr_4 = Label(cr_RegisterForm, text="Môn học 4", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=240)
    lbl_cr_5 = Label(cr_RegisterForm, text="Môn học 5", font=("Roboto", 8, 'bold'), bg='white').place(x=16, y=275)
    ...
    btn_show = ttk.Button(cr_RegisterForm, text="Làm mới", command=Read_cr, width = 20).place(x = 40,y = 320)
    #btn_show.pack()
    btn_add = ttk.Button(cr_RegisterForm, text="Thêm khóa học" ,command=add_book_cr,width = 20).place(x = 40,y = 350)
    #btn_add.pack()
    btn_exit = ttk.Button(cr_RegisterForm, text="Thoát", command = exit_register_cr,width = 20).place(x = 40,y = 380)
    #btn_exit.pack()
    #goto_TeacherHome
    ...
    scrollbarx.config(command=tree.xview)
    scrollbarx.pack(side=BOTTOM, fill=X)
    tree.heading('Course_No', text="ID", anchor=W)
...
def goto_AdminHome():
    ...
    screen_width = AdminRegisterForm.winfo_screenwidth()
    screen_height = AdminRegisterForm.winfo_screenheight()

    x_cordinate = int((screen_width/2) - (window_width/2))
    y_cordinate = int((screen_height/3) - (window_height/3))
    AdminRegisterForm.geometry("{}x{}+{}+{}".format(window_width, window_height, x_cordinate, y_cordinate))
    # AdminRegisterForm.geometry('1280x720')
    AdminRegisterForm.resizable(0,0)
    AdminRegisterForm.title("TRANG CHỦ QUẢN TRỊ")
    AdminRegisterForm.config(bg='white')
    AHframe = Frame(AdminRegisterForm, height=80, width=1280, bg='#3C6739').place(relx=0, y=0)
    LAdminTitle = Label(AdminRegisterForm, font=('Roboto', 30, 'bold'), text="TRANG CHỦ QUẢN TRỊ", bg='white').place(relx=0.35,
                                                                                                               y=150)
    B_ProfessorReg = Button(AdminRegisterForm, text='THÊM MỚI GIẢNG VIÊN', width=25, height=13, bg='#3C6739', fg='white',
                                      font=('Roboto', 10), bd=6,command = professor_register).place(x=120, rely=0.4)
    B_CourseReg = Button(AdminRegisterForm, text='THÊM MỚI KHÓA HỌC', width=25, height=13, bg='#3C6739', fg='white',
                                   font=('Roboto', 10), bd=6,command = course_register).place(x=520, rely=0.4)
    B_StudentReg = Button(AdminRegisterForm, text='THÊM MỚI SINH VIÊN', width=25, height=13, bg='#3C6739', fg='white',
                                       font=('Roboto', 10), bd=6,command = student_register).place(x=930, rely=0.4)
